##Longitudinal time to PIRA - split by AGEGROUP
...
df <- read.csv("/data/ms/processed/clinical/projects/msc-time2event/longitudinal-table-pira.csv")

#Subset the core phase of the study for each subject >>Core phase 
# df <- df %>% filter(CORE == "Yes")

colnames(df) <- gsub("T3MPIRA", "status", colnames(df))
