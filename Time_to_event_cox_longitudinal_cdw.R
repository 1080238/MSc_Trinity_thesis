...
pira_longitudinal_table <- read.csv("/data/ms/processed/clinical/projects/msc-time2event/longitudinal-table-pira.csv")
raw_longitudinal_table <- read.csv("/data/ms/processed/clinical/projects/msc-time2event/longitudinal-table-raw.csv")

# Merge the dataframes
merged_df <- full_join(pira_longitudinal_table, raw_longitudinal_table, by = c('USUBJID', 'STUDY', 'time1', 'time2', 'NBV', 'NUMGDT1SQRT', 'VOLT2CBRT', 'SEX', 'AGELG', 'DURFSLG', 'EDSSBL', 'RELPST1Y', 'RELPST2Y', 'ACTVTRT', 'T25FWM', 'HPT9M'))

merged_df$T3MPIRA[is.na(merged_df$T3MPIRA)] <- 0

merged_df$T3MRAW[is.na(merged_df$T3MRAW)] <- 0


# Create the T3MCDW event
merged_df <- merged_df %>%
  mutate(T3MCDW = ifelse((T3MPIRA == 1 | T3MRAW == 1), 1, 0))

# Group by USUBJID to find the earliest event onset or the maximum time2 if no event occurred
# Identify the relevant time2 for each subject
relevant_time2 <- merged_df %>%
  group_by(USUBJID) %>%
  summarize(relevant_time2 = if (any(T3MCDW == 1)) min(time2[T3MCDW == 1]) else max(time2), .groups = 'drop')

# Merge the relevant_time2 back to the original dataframe
merged_with_relevant_time2 <- merged_df %>%
  left_join(relevant_time2, by = "USUBJID")

# Filter the dataframe to include all rows up to the relevant time2 for each subject
cdw_final <- merged_with_relevant_time2 %>%
  filter(time2 <= relevant_time2) %>%
  select(-relevant_time2)

# Select relevant columns for the final dataframe
cdw_longitudinal_table <- cdw_final %>%
  select(USUBJID, STUDY, time1, time2, T3MCDW, NBV, NUMGDT1SQRT, VOLT2CBRT, SEX, AGELG, DURFSLG, EDSSBL, RELPST1Y, RELPST2Y, ACTVTRT, T25FWM, HPT9M)

# Display the result
print(cdw_longitudinal_table)

longitudinal_table <- cdw_longitudinal_table

write.csv(longitudinal_table, "/data/users/rty388/testing/longitudinal-table-cdw.csv", row.names = FALSE)
