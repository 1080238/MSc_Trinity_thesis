...
##Add in the brain reserve as a covariate!!! 
...
#Read in brain reserve data for each patient 
brain_res_data <- read.csv("/data/users/rty388/regression/Cox/Improvement/data/subIDs_dev_groups.csv")

brain_res_data <- brain_res_data[ ,2:3]
colnames(brain_res_data)[2] <- "Brain_reserve"

#Merge brain reserve data into dataframe
df <- df %>% merge(brain_res_data, by = "USUBJID")

# Assume your dataframe is named df and the column you want to replace is named 'column_name'
df <- df %>%
  mutate(Brain_reserve = case_when(
    Brain_reserve == 0  ~ "Medium",
    Brain_reserve == -1 ~ "Low",
    Brain_reserve == 1  ~ "High"
  ))

df$Brain_reserve <- factor(df$Brain_reserve, levels = c("Medium", "High", "Low"))
...
for (age_group_name in names(age_groups)) {
  ...
  for (event_type_name in names(event_types)) {
    ...
    day1_data <- df_subset %>%
      filter(DAY == 1) %>%
      select(USUBJID, SEX, AGE, DURFS, RELPST1Y, RELPST2Y, ACTVTRT, T25FWM, HPT9M, NBV, NUMGDT1squareroot, VOLT2cuberoot, EDSS, Brain_reserve) %>%
      distinct()
    ...
    cox_model <- coxph(Surv(time, status) ~ SEX + AGE + DURFS + RELPST1Y + RELPST2Y + ACTVTRT + T25FWM + HPT9M + NBV + NUMGDT1squareroot + VOLT2cuberoot + EDSS + Brain_reserve, data = df1)
    ...
    output_dir <- paste0("/data/users/rty388/regression/Cox/Worsening/round7/", event_type_name, "/", age_group_name)
    ...
    covariates <- c("SEX", "AGE", "DURFS", "RELPST1Y", "RELPST2Y", "ACTVTRT", 
                    "T25FWM", "HPT9M", "NBV", "NUMGDT1squareroot", "VOLT2cuberoot", "EDSS", "Brain_reserve")
    
    
    # Calculate mean and SD, ensuring proper handling of non-numeric covariates
    df1$Brain_reserve <- as.character(df1$Brain_reserve)
