...
for (age_group_name in names(age_groups)) {
  ...
  for (event_type_name in names(event_types)) {
    ...
    day1_data <- df_subset %>%
      filter(DAY == 1) %>%
      select(USUBJID, SEX, AGE, DURFS, RELPST1Y, RELPST2Y, ACTVTRT, T25FWM, HPT9M, NBV, NUMGDT1squareroot, VOLT2cuberoot, EDSS) %>%
      distinct()
    ...
    cox_model <- coxph(Surv(time, status) ~ SEX + AGE + DURFS + RELPST1Y + RELPST2Y + ACTVTRT + T25FWM + HPT9M + NBV + NUMGDT1squareroot + VOLT2cuberoot + EDSS, data = df1)
    ...
    output_dir <- paste0("/data/users/rty388/regression/Cox/Worsening/CrossSec/round6/", event_type_name, "/", age_group_name)
    ...
    covariates <- c("SEX", "AGE", "DURFS", "RELPST1Y", "RELPST2Y", "ACTVTRT", 
                    "T25FWM", "HPT9M", "NBV", "NUMGDT1squareroot", "VOLT2cuberoot", "EDSS")
    ...
    df1$VOLT2cuberoot <- as.numeric(df1$VOLT2cuberoot)
    
    df1 <- df1[!is.na(df1$VOLT2cuberoot), ]
    ...
    for (colour in colours) {
      ...
      surv_plot <- survfit2(cox_model) %>%
        ggsurvfit(color = colour) +  # Set the line color dynamically
        labs(
          x = "Days",
          y = paste("Overall probability of surving without", event_type_name)
        ) +
        add_confidence_interval(color = colour, fill = colour) +  # Set the CI color dynamically
        add_risktable(
          size = 4,
      ...
      ggsave(filename = paste0(output_dir, "/survfit_COX_curve_", event_type_name, "_", age_group_name, "_", colour, ".png"), plot = surv_plot, device = "png", width = 10, height = 8)
    }
      
      
      
      
     # Survival curve - regular plot
    surv_plot <- survfit2(cox_model) %>%
    ...
    #Plot risk plots with either blue or red figures: 
    colours <- c("blue", "red")
    ...
    for (colour in colours) {
      ...
      risk_plot <- ggplot(surv_df, aes(x = time, y = risk)) +
        ...
        geom_ribbon(aes(ymin = risk_lower, ymax = risk_upper), fill = colour, color = colour, alpha = 0.2) +
        labs(
          x = "Days",
          y = paste("Overall probability of", event_type_name)
        ) +
        theme_minimal() +
      ...
      ggsave(filename = paste0(output_dir, "/risk_curve_COX_", event_type_name, "_", age_group_name, colour, ".png"), plot = risk_plot, device = "png", width = 10, height = 8)
